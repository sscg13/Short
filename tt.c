...
long c_tt_probe = 0;                 /* tt_probe entries */
long c_tt_store = 0;                 /* tt_store entries */
#endif

#define TT_DEPTH(i)  ((int)((i) & 0x3F))
#define TT_FLAG(i)   ((int)((i) >> 6))
#define TT_INFO(d,f) ((unsigned char)(((d) & 0x3F) | ((f) << 6)))
...
static int tt_index(Sig key) {
    unsigned short w[4];
    memcpy(w, &key, 8);
    return (int)((w[0] ^ w[1] ^ w[2] ^ w[3]) & TT_MASK);
}
