...
/* Q12 fixed-point ln(n) for n = 0..63 (0 for n < 2): round(ln(n) * 4096),
   max 16970 (fits u16). Same seed data the old Python-generated table used. */
static const u16 lmr_ln[64] = {
    0, 0, 2839, 4500, 5678, 6592, 7339, 7970,
    8517, 9000, 9431, 9822, 10178, 10506, 10810, 11092,
    11357, 11605, 11839, 12060, 12271, 12470, 12661, 12843,
    13017, 13185, 13345, 13500, 13649, 13792, 13931, 14066,
    14196, 14322, 14444, 14563, 14678, 14790, 14900, 15006,
    15110, 15211, 15309, 15406, 15500, 15592, 15682, 15770,
    15856, 15941, 16024, 16105, 16184, 16262, 16339, 16414,
            ...
            if (legal_move) {
                ...
                if (first) {
                    ...
                    /* PVS: zero-window search; re-search full window only if it beats alpha.
                       ...
                       LMR: a late quiet move at a non-PV, non-check node is searched at a
                       reduced depth from lmr_tab (never below depth 1 - a qsearch probe on
